...
window.THEME_ICON = "🔮";
window.THEME_SFX  = {"win":"epic"};
window.THEME_MUSIC = {"bpm":96,"root":45,"stepsPerBar":16,"chords":[[0,7,12,19],[3,10,15,19],[-2,5,10,17],[-4,3,7,14]],"pad":{"wave":"sawtooth","vol":0.055,"oct":1,"dur":1.06,"atk":0.52,"cut":1500},"bass":{"pat":[1,0,0,0,1,0,0,0],"oct":0,"vol":0.085,"wave":"triangle","dur":4,"cut":300},"arp":{"pat":[0,2,1,3,4,3,1,2],"wave":"triangle","vol":0.032,"oct":2,"dur":1.4},"drums":"taiko","drumVol":0.6,"swing":0,"reverb":0.65,"delay":0.3,"bells":true};
...
function tpLayout() {
  ...
  var r = TC.autoFitGrid(tpNames, tpCells.length, {
  ...
  for (var i = 0; i < tpCells.length; i++) {
    ...
    var fs = Math.min(r.ch * 0.66, r.cw * 1.72 / len, r.cw * 0.90);
    if (fs < 12) fs = 12;
    nm.style.fontSize = Math.round(fs) + 'px';
    id3.style.fontSize = Math.max(7, Math.round(fs * 0.26)) + 'px';
  }
}
...
function revealWinners(winners) {
  ...
  for (var i = 0; i < tpCells.length; i++) {
    ...
    cell.querySelector('.nm').textContent = w.name;
    cell.querySelector('.id3').textContent = 'No.' + w.id;
    var len = Math.max(1, Math.min(8, w.name.length));
    var fs = Math.min(r.ch * 0.66, r.cw * 1.72 / len, r.cw * 0.90);
    if (fs < 12) fs = 12;
    cell.querySelector('.nm').style.fontSize = Math.round(fs) + 'px';
    cell.querySelector('.id3').style.fontSize = Math.max(7, Math.round(fs * 0.26)) + 'px';
  }
