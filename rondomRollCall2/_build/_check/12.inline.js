...
window.THEME_MUSIC = {"bpm":116,"root":57,"stepsPerBar":16,"chords":[[0,4,7,11],[7,11,14,18],[9,13,16,20],[5,9,12,16]],"pad":{"wave":"triangle","vol":0.055,"oct":1,"dur":1.06,"atk":0.46,"cut":2600},"bass":{"pat":[1,0,0,0,0,0,1,0],"oct":0,"vol":0.08,"wave":"triangle","dur":3,"cut":440},"arp":{"pat":[0,2,1,3,4,3,1,2],"wave":"square","vol":0.03,"oct":2,"dur":0.62},"drums":"clap","drumVol":0.55,"swing":0,"reverb":0.34,"delay":0.28,"bells":true};
...
function bbLayout() {
  ...
  var r = TC.autoFitGrid(bbCards, bbCardsEls.length, {
    gap: Math.max(7, Math.min(20, bbCards.clientWidth / 110)),
    ratio: 1.30, minW: 92, maxW: 260, pad: 6
  });
  if (!r) return;
  for (var i = 0; i < bbCardsEls.length; i++) {
    ...
    var fs = Math.min(r.ch * 0.30, r.cw * 1.70 / len, r.cw * 0.42);
    if (fs < 12) fs = 12;
    nm.style.fontSize = Math.round(fs) + 'px';
    id2.style.fontSize = Math.max(8, Math.round(fs * 0.42)) + 'px';
    if (em) em.style.fontSize = Math.max(10, Math.round(fs * 0.52)) + 'px';
  }
}
